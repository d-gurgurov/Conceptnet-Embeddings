...
class PPMIGenerator:
    ...
    def process_conceptnet_data(self, data):
        ...
        for word, relationships in data.items():
            related_words = set()
            # word = word.split(f"/c/{self.language}/")[1]
            for relation in relationships:
                if relation['surfaceStart'] != word:
                    related_words.add(relation['surfaceStart'])
                elif relation['surfaceEnd'] != word:
                    related_words.add(relation['surfaceEnd'])
            conceptnet_data[word] = list(related_words)

        conceptnet_data = {key.replace('_', ' '): value for key, value in conceptnet_data.items()}
